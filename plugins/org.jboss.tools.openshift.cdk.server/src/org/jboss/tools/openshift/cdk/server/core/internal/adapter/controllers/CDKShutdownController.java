...
public class CDKShutdownController extends AbstractSubsystemController implements IServerShutdownController, IExternalLaunchConstants {
	...
	public void stop(boolean force) {
		...
		int credType = cdkServer.getCredentialType();
		if( getRequiresForce() || credType == CDKServer.CREDENTIAL_OPTION_PROMPT ) {
			shutdownViaTerminal(requiresCancelation);
		} else {
	...
	private void shutdownViaExternalTools() {
		final IServer s = getServer();
		final CDKServer cdkServer = (CDKServer)s.loadAdapter(CDKServer.class, new NullProgressMonitor());
		final ControllableServerBehavior beh = (ControllableServerBehavior)s.loadAdapter(ControllableServerBehavior.class, new NullProgressMonitor());
		
		int credType = cdkServer.getCredentialType();
		ILaunchConfigurationWorkingCopy wc = null;
		try {
			wc = findLaunchConfig(s);
		} catch(CoreException ce) {
			// TODO log
		}
		if( wc == null ) {
			// TODO something wrong?  
			shutdownViaTerminal(true);
			return;
		}
		...
		if( credType == CDKServer.CREDENTIAL_OPTION_ENV_VAR) {
		...
		wc.setAttribute(ATTR_ARGS, CDKConstants.VAGRANT_CMD_HALT);
	...
	private void shutdownViaTerminal(boolean requiresCancelation) {
		...
		if( type == CDKServer.CREDENTIAL_OPTION_ENV_VAR) {
			command2 = CDKConstants.CDK_ENV_SUB_USERNAME + "=" + cdkServer.getUsername() + " " + 
						CDKConstants.CDK_ENV_SUB_PASSWORD + "=" + cdkServer.getPassword() + " " + command2;
		}
